...
internal class KafkaMessagePatternTest {
    ...
    fun `should match a kafka message`() {
        val kafkaMessagePattern = KafkaMessagePattern("target", StringPattern, NumberTypePattern)
    ...
    fun `should generate multiple message types when optionals are in the payload`() {
        val patterns = KafkaMessagePattern("target", StringPattern, AnyPattern(listOf(NumberTypePattern, NullPattern))).newBasedOn(Row(), Resolver())
        assertThat(patterns).hasSize(2)
        assertThat(patterns[0].value).isEqualTo(NumberTypePattern)
        assertThat(patterns[1].value).isEqualTo(NullPattern)
    }

    @Test
    fun `should encompass itself`() {
        val pattern = KafkaMessagePattern("target", StringPattern, AnyPattern(listOf(NumberTypePattern, NullPattern)))
    ...
    fun `should encompass a pattern with a smaller message`() {
        val bigger = KafkaMessagePattern("target", StringPattern, AnyPattern(listOf(NumberTypePattern, NullPattern)))
        val smallerNull = KafkaMessagePattern("target", StringPattern, AnyPattern(listOf(NullPattern)))
        val smallerNumber = KafkaMessagePattern("target", StringPattern, AnyPattern(listOf(NumberTypePattern)))
