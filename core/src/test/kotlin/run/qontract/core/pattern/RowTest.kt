...
internal class RowTest {
    ...
    fun `returns the value returned from another contract`() {
        val references = References("user", "user.qontract", valuesCache = mapOf("name" to "Jane"))
        val row = Row(listOf("name"), listOf("(=user.name)"), references = mapOf("user" to references))
        assertThat(row.getField("name")).isEqualTo("Jane")
    }
