...
private fun loadWsdlExamples(wsdl: WSDL, examplesDir: File): Map<String, List<Pair<HttpRequest, HttpResponse>>> {
    ...
    for (operationName in operationNames) {
        val responseXmlFile = File(examplesDir, "$operationName.xml")
        val requestXmlFile = File(examplesDir, "${operationName}_request.xml")
        
        if (responseXmlFile.exists()) {
            val responseXmlContent = responseXmlFile.readText()
            
            // Use request XML if available, otherwise use empty body
            val requestBody = if (requestXmlFile.exists()) {
                requestXmlFile.readText()
            } else {
                "" // Will be generated by the contract test
            }
            ...
            val request = HttpRequest(
                ...
                body = if (requestBody.isNotEmpty()) StringValue(requestBody) else EmptyString
            )
            
            val response = HttpResponse(
                status = 200,
                body = responseXmlContent,
