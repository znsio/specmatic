...
fun toGherkinClauses(kafkaMessage: KafkaMessage): List<GherkinClause> {
    ...
    val newTypes = valueTypeDeclaration.types.plus(keyTypeDeclaration?.types ?: emptyMap())
    val gherkinTypeDeclarations = run.qontract.core.toGherkinClauses(newTypes)

    val gherkinContent = listOfNotNull("kafka-message ${kafkaMessage.topic}", keyTypeDeclaration?.typeValue, valueTypeDeclaration.typeValue).joinToString(" ")
