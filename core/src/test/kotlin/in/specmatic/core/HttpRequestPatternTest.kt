...
//        println(negativeTestScenarios.map { it.second }.filterIsInstance<HasValue<*>>().forEach(" ") { it.valueDetails.joinToString})
        negativeTestScenarios.map { it.second }.filterIsInstance<HasValue<Scenario>>().map { it.value }.forEach { it: Scenario ->
