...
internal class HttpRequestPatternTest {
    ...
    fun `a clone request pattern request should include the headers specified`() {
        val pattern = HttpRequestPattern(
                ...
                httpUrlPattern = toURLMatcherWithOptionalQueryParams(URI("/")),
                method = "GET"
        )
    ...
    fun `number bodies should match numerical strings`() {
        val requestPattern = HttpRequestPattern(method = "GET", httpUrlPattern = toURLMatcherWithOptionalQueryParams("/"), body = NumberPattern())
    ...
    fun `boolean bodies should match boolean strings`() {
        val requestPattern = HttpRequestPattern(method = "GET", httpUrlPattern = toURLMatcherWithOptionalQueryParams("/"), body = BooleanPattern())
    ...
    fun `request with multiple parts and no optional values should result in just one test for the whole`() {
        ...
        val requestPattern = HttpRequestPattern(method = "GET", httpUrlPattern = toURLMatcherWithOptionalQueryParams("/"), multiPartFormDataPattern = parts)
    ...
    fun `request with an optional part should result in two requests`() {
        ...
        val requestPattern = HttpRequestPattern(method = "GET", httpUrlPattern = toURLMatcherWithOptionalQueryParams("/"), multiPartFormDataPattern = listOf(part))
        val patterns = requestPattern.newBasedOn(Row(), Resolver())

        assertThat(patterns).hasSize(2)

        assertThat(patterns).contains(HttpRequestPattern(method = "GET", httpUrlPattern = toURLMatcherWithOptionalQueryParams("/"), multiPartFormDataPattern = emptyList()))
        assertThat(patterns).contains(HttpRequestPattern(method = "GET", httpUrlPattern = toURLMatcherWithOptionalQueryParams("/"), multiPartFormDataPattern = listOf(part.nonOptional())))
    }
    ...
    fun `request with a part json body with a key in a row should result in a request with the row value`() {
        ...
        val expectedPattern = HttpRequestPattern(method = "GET", httpUrlPattern = toURLMatcherWithOptionalQueryParams("/"), multiPartFormDataPattern = listOf(MultiPartContentPattern(
            "data",
    ...
    fun `request type having an optional part name should match a request in which the part is missing`() {
        ...
        val requestType = HttpRequestPattern(method = "GET", httpUrlPattern = toURLMatcherWithOptionalQueryParams("/"), multiPartFormDataPattern = listOf(part))
    ...
    fun `should generate a request with an array body if the array is in an example`() {
        ...
        val requestType = HttpRequestPattern(httpUrlPattern = toURLMatcherWithOptionalQueryParams("/"), body = parsedPattern("(body: RequestBody)"))
    ...
    fun `should generate a stub request pattern from an http request in which the query params are not optional`() {
        val requestType = HttpRequestPattern(method = "GET", httpUrlPattern = HttpURLPattern(mapOf("status" to StringPattern()), pathToPattern("/"), "/"))
        val newRequestType = requestType.generate(HttpRequest("GET", "/", queryParams = mapOf("status" to "available")), Resolver())

        assertThat(newRequestType.httpUrlPattern?.queryPatterns?.keys?.sorted()).isEqualTo(listOf("status"))
    ...
    fun `form field of type json in string should match a form field value of type json in string`() {
        ...
        HttpRequestPattern(
            method = "POST",
            httpUrlPattern = HttpURLPattern(emptyMap(), emptyList(), "/"),
    ...
    fun `match errors across the request including header and body will be returned`()  {
        val type = HttpRequestPattern(method = "POST", httpUrlPattern = toURLMatcherWithOptionalQueryParams("http://helloworld.com/data"), headersPattern = HttpHeadersPattern(mapOf("X-Data" to NumberPattern())), body = JSONObjectPattern(mapOf("id" to NumberPattern())))
    ...
    fun `should lower case header keys while loading stub data`()  {
        val type = HttpRequestPattern(method = "POST", httpUrlPattern = toURLMatcherWithOptionalQueryParams("http://helloworld.com/data"), headersPattern = HttpHeadersPattern(mapOf("x-data" to StringPattern())), body = JSONObjectPattern(mapOf("id" to NumberPattern())))
        val request = HttpRequest("POST", "/data", headers = mapOf("X-Data" to "abc123"), body = parsedJSON("""{"id": "abc123"}"""))
    ...
    inner class MultiPartMatchReturnsAllErrors {
        ...
        val requestPattern = HttpRequestPattern(method = "POST", httpUrlPattern = toURLMatcherWithOptionalQueryParams("/"), multiPartFormDataPattern = parts)
    ...
    fun `should not generate test request for generative tests more than once`()  {
        val pattern = HttpRequestPattern(method = "POST", httpUrlPattern = toURLMatcherWithOptionalQueryParams("http://helloworld.com/data"), body = JSONObjectPattern(mapOf("id" to NumberPattern())))
