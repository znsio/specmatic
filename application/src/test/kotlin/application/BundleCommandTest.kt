...
internal class BundleCommandTest {
    ...
    fun `the command should compress all stub commands in the config into a zip file`() {
        ...
        every { fileReader.readBytes(contractPath) }.returns(bytes)
        ...
        every { fileReader.readBytes(stubFilePath) }.returns(bytes)

        every { fileReader.files("/Users/jane_doe/.qontract/repos/git-repo/com/example/api_1_data") }.returns(files)
        ...
        verify(exactly = 1) { fileReader.readBytes(stubFilePath) }
