...
"""
    ...
    @Test
    fun `request with a string rest should result in a string rest pattern`() {
        val request = HttpRequest("POST", "/", emptyMap(), parsedValue("""{"data": ["(string...)"]}"""))
        val requestPattern = request.toPattern()
        val body = requestPattern.body
    ...
    @Test
    fun `when request body is string question it should result in nullable string pattern as body`() {
