...
class SpringRequestMappingKtTest {
    @Test
    fun `convert common brackets to braces when designating sigle path parameters in url`() {
        val path = "/(country:string)/state"
        assertEquals("/{country}/state", convertPathParameterStyle(path))
    }

    @Test
    fun `convert common brackets to braces when designating double path parameters in url`() {
        val path = "/(country:string)/state/(state:string)"
        assertEquals("/{country}/state/{state}", convertPathParameterStyle(path))
    }

    @Test
    fun `convert common brackets to braces when designating multiple path parameters in url`() {
